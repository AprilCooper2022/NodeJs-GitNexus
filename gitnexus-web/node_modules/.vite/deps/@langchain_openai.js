import { At as FunctionMessageChunk, E as ZodFirstPartyTypeKind, Et as HumanMessageChunk, K as patchConfig, Lt as AIMessage, M as GenerationChunk, Mn as ContextOverflowError, Nt as ChatMessage, Pt as ChatMessageChunk, Rt as AIMessageChunk, Ut as ToolMessage, Vn as getSchemaDescription, Wt as ToolMessageChunk, Y as AsyncLocalStorageProviderSingleton, _n as parseMimeType, a as RunnableLambda, b as toJsonSchema, dr as isZodSchemaV3, dt as getEnvironmentVariable, er as isInteropZodSchema, fn as isDataContentBlock, fr as isZodSchemaV4, gn as parseBase64DataUrl, gt as iife, j as ChatGenerationChunk, q as pickRunnableConfigKeys, un as convertToProviderContentBlock, ut as getEnv, w as isSerializableSchema, xt as SystemMessageChunk, zt as isAIMessage } from "./base-DI83fCzA.js";
import { F as toJSONSchema, gi as parse } from "./core-D_QZK_Gx.js";
import { _ as makeInvalidToolCall, b as JsonOutputParser, d as createContentParser, f as createFunctionCallingParser, g as convertLangChainToolCallToOpenAI, i as BaseChatModel, n as convertToOpenAITool, s as finalizeContentBlock, u as assembleStructuredOutputPipeline, v as parseToolCall$2 } from "./function_calling-DFXUtcum.js";
import "./messages-Daki7zQz.js";
import { Et as array, Nn as record, Rn as string, Tn as object, Xn as union, dn as literal, wn as number, yt as _enum } from "./schemas-Cbyr_kbb.js";
import { c as calculateMaxTokens, l as isOpenAITool, t as isLangChainTool } from "./types-DYLcZHO_.js";
import { DynamicTool, Tool, tool } from "./@langchain_core_tools.js";
import { t as Embeddings$1 } from "./embeddings-CP2VJFEI.js";
import { t as chunkArray } from "./chunk_array-CHJR8OY9.js";
import { n as convertOpenAICompletionsStream } from "./openai_tools-Cxf4XkMj.js";
import { t as BaseLLM } from "./llms-DxWiaxtv.js";
...
function toStrictJsonSchema(schema) {
	if (schema.type !== "object") throw new Error(`Root schema must have type: 'object' but got type: ${schema.type ? `'${schema.type}'` : "undefined"}`);
...
function isNullable(schema) {
	...
	for (const oneOfVariant of schema.oneOf ?? []) if (isNullable(oneOfVariant)) return true;
	for (const allOfVariant of schema.anyOf ?? []) if (isNullable(allOfVariant)) return true;
	return false;
}
/**
...
function ensureStrictJsonSchema(jsonSchema, path, root) {
	...
	if (isObject(defs)) for (const [defName, defSchema] of Object.entries(defs)) ensureStrictJsonSchema(defSchema, [
		...path,
		"$defs",
		defName
	], root);
	const definitions = jsonSchema.definitions;
	if (isObject(definitions)) for (const [definitionName, definitionSchema] of Object.entries(definitions)) ensureStrictJsonSchema(definitionSchema, [
		...path,
		"definitions",
		definitionName
	], root);
	if (jsonSchema.type === "object" && !("additionalProperties" in jsonSchema)) jsonSchema.additionalProperties = false;
	const required = jsonSchema.required ?? [];
	const properties = jsonSchema.properties;
	if (isObject(properties)) {
		for (const [key, value] of Object.entries(properties)) if (!isNullable(value) && !required.includes(key)) throw new Error(`Zod field at \`${[
			...path,
			"properties",
			key
		...
		jsonSchema.properties = Object.fromEntries(Object.entries(properties).map(([key, propSchema]) => [key, ensureStrictJsonSchema(propSchema, [
			...path,
			"properties",
			key
		], root)]));
	...
	if (isObject(items)) jsonSchema.items = ensureStrictJsonSchema(items, [...path, "items"], root);
	const anyOf = jsonSchema.anyOf;
	if (Array.isArray(anyOf)) jsonSchema.anyOf = anyOf.map((variant, i) => ensureStrictJsonSchema(variant, [
		...path,
		"anyOf",
		String(i)
	], root));
	const allOf = jsonSchema.allOf;
	if (Array.isArray(allOf)) if (allOf.length === 1) {
	...
	} else jsonSchema.allOf = allOf.map((entry, i) => ensureStrictJsonSchema(entry, [
		...path,
		"allOf",
		String(i)
	], root));
	...
	if (ref && hasMoreThanNKeys(jsonSchema, 1)) {
		...
		if (typeof resolved === "boolean") throw new Error(`Expected \`$ref: ${ref}\` to resolve to an object schema but got boolean`);
		if (!isObject(resolved)) throw new Error(`Expected \`$ref: ${ref}\` to resolve to an object but got ${JSON.stringify(resolved)}`);
...
function resolveRef(root, ref) {
	if (!ref.startsWith("#/")) throw new Error(`Unexpected $ref format ${JSON.stringify(ref)}; Does not start with #/`);
	const pathParts = ref.slice(2).split("/");
	let resolved = root;
	for (const key of pathParts) {
		if (!isObject(resolved)) throw new Error(`encountered non-object entry while resolving ${ref} - ${JSON.stringify(resolved)}`);
		const value = resolved[key];
		if (value === void 0) throw new Error(`Key ${key} not found while resolving ${ref}`);
		resolved = value;
	}
	return resolved;
}
...
var BaseChatOpenAI = class extends BaseChatModel {
	...
	*/
	...
	constructor(fields) {
		...
		this._addVersion("@langchain/openai", "1.5.0");
	}
	/**
...
//#endregion
//#region node_modules/@langchain/openai/dist/azure/chat_models/common.js
var AZURE_ALIASES = {
...
//#endregion
//#region node_modules/@langchain/openai/dist/azure/chat_models/completions.js
...
//#endregion
//#region node_modules/@langchain/openai/dist/azure/chat_models/responses.js
...
//#endregion
//#region node_modules/@langchain/openai/dist/azure/chat_models/index.js
/**
...
//#endregion
//#region node_modules/@langchain/openai/dist/llms.js
/**
...
//#endregion
//#region node_modules/@langchain/openai/dist/azure/llms.js
...
//#endregion
//#region node_modules/@langchain/openai/dist/embeddings.js
/**
...
* ```
*/
var OpenAIEmbeddings = class extends Embeddings$1 {
...
//#endregion
//#region node_modules/@langchain/openai/dist/azure/embeddings.js
...
//#endregion
//#region node_modules/@langchain/openai/dist/tools/dalle.js
/**
...
//#endregion
//#region node_modules/@langchain/openai/dist/tools/webSearch.js
/**
...
//#endregion
//#region node_modules/@langchain/openai/dist/tools/mcp.js
/**
...
//#endregion
//#region node_modules/@langchain/openai/dist/tools/codeInterpreter.js
/**
...
//#endregion
//#region node_modules/@langchain/openai/dist/tools/fileSearch.js
/**
...
//#endregion
//#region node_modules/@langchain/openai/dist/tools/imageGeneration.js
/**
...
var ComputerUseActionSchema = object({ action: union([
...
var LocalShellActionSchema = union([object({
	type: literal("exec"),
...
//#endregion
//#region node_modules/@langchain/openai/dist/tools/shell.js
...
var ApplyPatchOperationSchema = union([
	object({
...
//#endregion
//#region node_modules/@langchain/openai/dist/tools/toolSearch.js
/**
...
//#endregion
//#region node_modules/@langchain/openai/dist/tools/index.js
var tools = {
	webSearch,
	mcp,
	codeInterpreter,
...
//#endregion
//#region node_modules/@langchain/openai/dist/tools/custom.js
...
//#endregion
//#region node_modules/@langchain/openai/dist/utils/prompts.js
/**
